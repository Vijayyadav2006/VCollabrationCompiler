...
const switchFile = (index) => {
  ...
  setLanguage(file.language);       // ✅ sync global language
  setSelectedLanguage(file.language); // ✅ sync dropdown
...
useEffect(() => {
  ...
  if (savedFiles) {
    ...
    if (editorRef.current && parsedFiles.length > 0) {
      const firstLang = parsedFiles[0].language || "c";
...
typingSocketRef.current.on("initialTypers", (activeTypers) => {
 ...
 const changeLanguage = (language) => {
  ...
  if (editorRef.current) {
    editorRef.current.session.setMode(modeMapping[language] || "ace/mode/c_cpp");
  }
};
...
/>
        ...
        <div className="col-md-10 d-flex flex-column h-100">
          ...
          <div className="control-panel">
            ...
            <select
  id="language-select"
  value={files[activeFileIndex]?.language || "c"}
  onChange={(e) => {
    const newLang = e.target.value; // ✅ define newLang here

    setLanguage(newLang);
    setSelectedLanguage(newLang);

    // ✅ optional extension update (if you want filename to match language)
    setFiles((prevFiles) =>
      prevFiles.map((file, index) => {
        if (index === activeFileIndex) {
          ...
          return { ...file, language: newLang, name: `${nameWithoutExt}.${newExt}` };
        }
        return file;
      })
    );
